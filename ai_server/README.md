...
## 8️⃣ 🚀 Start AI Server (Backend API)

Sau khi đã cài đặt dependencies và đăng ký dữ liệu khuôn mặt, tiến hành khởi động AI Server:
...
- `modules.face_recognition.backend.AI_backend:app`
  - `AI_backend.py` là file chứa FastAPI instance
  - `app` là object FastAPI

- `--host 0.0.0.0`
  - Cho phép server truy cập từ các thiết bị khác trong cùng mạng (LAN)
...
## 9️⃣ 🎥 Start CAM Backend (Realtime Recognition)

Sau khi AI Server đã chạy, mở **một terminal mới** để khởi động CAM Backend.

---

### 1️⃣ Di chuyển vào thư mục ai_server
...
### 3️⃣ Chạy CAM Backend

```bash
python -m modules.face_recognition.backend.CAM_backend --location house_A
```
...
- `--location house_A`
  - Phải trùng với location đã nhập khi đăng ký khuôn mặt.
  - CAM Backend sẽ chỉ nhận diện các embedding thuộc location này.
...
1. Camera bật lên.
2. Frame được gửi lên AI Server.
3. AI Server:
   - Detect face
   - Extract embedding
...
4. CAM Backend:
   - Nhận kết quả
   - Gán track_id bằng DeepSORT
   - Hiển thị name + similarity + track_id lên màn hình

---

💡 Lưu ý:

- AI Server phải đang chạy trước khi bật CAM Backend.
- Đảm bảo `location` truyền vào đúng với location khi đăng ký.
- Nếu không nhận diện được, kiểm tra:
  - Database đã có dữ liệu chưa
  - Location có đúng không
  - AI Server có đang chạy không
